...
namespace SmartScalesApp.ConsoleUI
{
    public class ConsoleUIMenus
    {
        ...
        private StatsTotalWeightLossServices? _statsTotalWeightLossService;
        public bool Run()
        {
            Console.Clear();
        ...
        public void CalculateStats()
        {
            ...
            ConsoleOutput.WriteLine(ConsoleColor.DarkCyan, "3) User total weight loss");
            ConsoleOutput.WriteLine(ConsoleColor.DarkCyan, "4) Back to the main menu");
            var option = ConsoleInput.ReadString(ConsoleColor.Blue, "\r\nSelect an option: ");
        ...
        public void CalculateTotalWeightLoss()
        {
            Console.Clear();
            _userProfileService = GetUserProfileServices();
            _statsTotalWeightLossService = GetStatsTotalWeightLossService();

            var userprofile = SelectUserProfile(ConsoleColor.Blue, "Please select the user profile to calculate total weight loss for:", _userProfileService.GetUserProfiles());
            var totalWeightLossResult = _statsTotalWeightLossService.GetTotalWeightLossResult(userprofile.UserID);

            Console.Clear();
            var table = new ConsoleTable();
            table.Columns.Clear();
            table.Columns.Add("Total weight loss");
            table.Columns.Add("Weight measure");

            table.AddRow(totalWeightLossResult.TotalWeightLoss, totalWeightLossResult.TotalWeightLossWeightMeasure);
        ...
        private StatsTotalWeightLossServices GetStatsTotalWeightLossService()
        {
            return new StatsTotalWeightLossServices(new UserProfileRepository("Data/DataFiles/UserProfiles"),
                                         new WeightRecordRepository("Data/DataFiles/WeightRecords"));
        }
